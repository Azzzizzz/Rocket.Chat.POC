...
- `POST /subscriptions.read` — Mark a room as read (clears unread count for the current user).
...
- `sub stream-notify-user` — Subscribe to user notifications:
  - `${userId}/subscriptions-changed` — Subscription updates including unread changes.
  - `${userId}/rooms-changed` — Room updates including unread changes.

## Totals

- REST endpoints: 19
- Realtime actions: 5 (connect, login, room-messages, notify-user: subscriptions, rooms)
